DemoData <- read.csv(file = "./DemoData.csv", header = TRUE, row.names = NULL, encoding = "UTF-8", sep = ",", dec = ".", quote = "", comment.char = "")
...
arima.single.id <- function(data) {
  ...
  saveRDS( train.model,"./arimamodel.rda")
  
  newmodel <- readRDS("./arimamodel.rda")
  forecast.value <- newmodel$mean
